...
require 'config.php';
    ...
    if(empty(trim($_POST["new_salasana"]))){
        header("location: ../vaihdass.php?error=syotasalasana");
		exit();   
    } elseif(strlen(trim($_POST["new_salasana"])) < 6){
        header("location: ../vaihdass.php?error=salasanaliianlyhyt");
		exit();
    } else{
    ...
    if(empty(trim($_POST["confirm_salasana"]))){
        header("location: ../vaihdass.php?error=toistasalasana");
		exit();
    } else{
        $confirm_salasana = trim($_POST["confirm_salasana"]);
        if(empty($new_salasana_err) && ($new_salasana != $confirm_salasana)){
            header("location: ../vaihdass.php?error=salasanateivattasmaa");
			exit();
        }
    }
        ...
        if($stmt = mysqli_prepare($yhteys, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: ../asiakas/omattiedot.php?success");
                exit();
            } else{
	...
	header("location: ../asiakas/omattiedot.php?error");
    exit();
}
?>